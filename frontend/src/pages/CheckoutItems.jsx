...
const CheckoutItems = () => {
  ...
  const openModal = async (category) => {
    ...
    setRemoveQty({});
    try {
      const res = await axios.get('https://foodinventoryapp.onrender.com/foods');
  ...
  const handleRemove = async (item) => {
    ...
    try {
      if (qtyToRemove === item.quantity) {
        // Delete the item
        await axios.delete(`https://foodinventoryapp.onrender.com/foods/${item._id}`);
        ...
        await axios.put(`https://foodinventoryapp.onrender.com/foods/${item._id}`, updatedItem);
        setItems(items =>
          items.map(i =>
